...
export class MainMenu extends LitElement {
	...
	fetchData(baseURL, menuID) {
		...
		fetch(url, {})
			...
			.then((json) => {
				try {
					console.log(denormalize(menuID));
					const denormalized = denormalize(menuID);
	...
	renderAzMenuItem(item) {
		...
		const hierarchy = item?.link?.attributes?.["drupal-menu-hierarchy"];

		// Determine if the item is a top-level menu item
		const isTopLevel = hierarchy && hierarchy.length === 1;

		// Adjust href to include the base URL if needed
		href = href.charAt(0) === "/" ? this.thisUrl + href : href;

		// Handle top-level items with no children (like News and Athletics)
		if (isTopLevel && (!children || children.length === 0)) {
			return `<li part="menu-item" class="nav-item"><a class="nav-link" href="${href}">${title}</a></li>`;
		}

		// Handle parent menu items with children (like Admissions, Academics, etc.)
		if (isTopLevel && children && children.length) {
			let childItemsHtml = children
				.map((child) => {
					// Assuming 'child' has similar structure as 'item'
					return `<a part="menu-item" class="dropdown-item" href="${child.link.href}">${child.link.attributes.title}</a>`;
				})
				.join("");
		...
		// Handle other non-top-level items
		if (!isTopLevel && href) {
			return `<a part="menu-item" class="dropdown-item" href="${href}">${title}</a>`;
		}
	...
	// 	href =
	// 		item?.link?.href.charAt(0) === "/"
	// 			? this.thisUrl + item?.link?.href
	// 			: item?.link?.href;
...
customElements.get("az-main-menu") ||
	customElements.define("az-main-menu", MainMenu);
